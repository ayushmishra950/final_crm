...
import { setAdminStats, setAdminProviders, setAdminUsers, setAdminReviews, setAdminData } from '../../redux-toolkit/slice/adminSlice';
...
export default function AdminDashboard() {
   ...
   useEffect(() => {
      ...
      socket.on('admin_update', () => {
         console.log("Socket: Admin update received, refreshing data...");
         fetchDashboardData();
      });
            ...
            <div className="admin-main">
               ...
               <div className="admin-header">
                  <div>
                     <h2 style={{ fontSize: '1.25rem', margin: 0, fontWeight: 600 }}>
                        {activeTab === 'dashboard' ? 'Control Panel' : activeTab === 'providers' ? 'Network & KYC' : activeTab === 'users' ? 'User Management' : activeTab === 'categories' ? 'Service Categories' : activeTab === 'profile' ? 'Admin Profile' : 'Trust & Moderation'}
                     </h2>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                     <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', paddingRight: '1rem', borderRight: '1px solid #e2e8f0' }}>
                        <img src={`https://ui-avatars.com/api/?name=${adminData?.name || adminData?.fullName || 'Admin'}&background=4f46e5&color=fff`} style={{ width: '36px', height: '36px', borderRadius: '50%' }} />
                        <div className="hidden-mobile">
                           <div style={{ fontSize: '0.875rem', fontWeight: 600 }}>{adminData?.name || adminData?.fullName || 'Superadmin'}</div>
                        </div>
                     </div>
               ...
               <div className="admin-content">
                  ...
                  {activeTab === 'profile' && adminData && (
                     <div className="animate-fade-in">
                        <div className="admin-card" style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', padding: '3rem 2rem' }}>
                           <img src={`https://ui-avatars.com/api/?name=${adminData.name || adminData.fullName}&background=4f46e5&color=fff&size=128`} style={{ borderRadius: '50%', marginBottom: '1.5rem', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)' }} alt="Admin Profile" />
                           <h2 style={{ fontSize: '1.75rem', color: '#1e293b', margin: '0 0 0.5rem 0' }}>{adminData.name || adminData.fullName}</h2>
                           <div className="admin-badge" style={{ background: '#eef2ff', color: '#4f46e5', fontSize: '0.875rem', padding: '0.4rem 1rem', marginBottom: '2rem' }}>
                              <ShieldCheck size={16}/> Super Administrator
                           </div>

                           <div style={{ textAlign: 'left', background: '#f8fafc', borderRadius: '12px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '1rem', borderBottom: '1px solid #e2e8f0' }}>
                                 ...
                                 <span style={{ color: '#1e293b', fontWeight: 600 }}>{adminData.email}</span>
                              </div>
                              <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '1rem', borderBottom: '1px solid #e2e8f0' }}>
                                 <span style={{ color: '#64748b', fontWeight: 500 }}>Phone Number</span>
                                 <span style={{ color: '#1e293b', fontWeight: 600 }}>{adminData.phone || adminData.mobile || 'Not specified'}</span>
                              </div>
                              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                 <span style={{ color: '#64748b', fontWeight: 500 }}>Account Status</span>
                                 <span style={{ color: '#10b981', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.25rem' }}><CheckCircle size={16}/> Active & Verified</span>
                              </div>
                           </div>
                        </div>
                     </div>
