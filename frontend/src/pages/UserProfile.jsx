...
export default function UserProfile() {
  ...
  const [user, setUser] = useState({ name: 'Rahul Verma', phone: '+91 98765 43210', email: 'rahul@example.com' });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f8fafc' }}>
      ...
      <div className="navbar" style={{ padding: '1.25rem 1rem', background: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
         ...
         <button className="btn-icon" onClick={() => setIsEditing(true)} style={{ background: '#f1f5f9', border: 'none', color: '#4f46e5', cursor: 'pointer' }}>
            <Edit3 size={18} />
         </button>
      </div>
            ...
            <div className="animate-fade-in" style={{ background: 'white', width: '100%', maxWidth: '400px', borderRadius: '24px', padding: '2rem 1.5rem', position: 'relative' }}>
               ...
               <form onSubmit={(e) => { e.preventDefault(); setIsEditing(false); }}>
                  <div className="input-group" style={{ marginBottom: '1rem' }}>
                     ...
                     <input type="text" className="input-field" value={user.name} onChange={(e) => setUser({...user, name: e.target.value})} style={{ width: '100%', padding: '0.75rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px' }} required />
                  </div>
                  <div className="input-group" style={{ marginBottom: '1rem' }}>
                     <label style={{ fontSize: '0.875rem', fontWeight: 500, color: '#334155', display: 'block', marginBottom: '0.5rem' }}>Mobile Number</label>
                     <input type="tel" className="input-field" value={user.phone} onChange={(e) => setUser({...user, phone: e.target.value})} style={{ width: '100%', padding: '0.75rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px' }} required />
                  </div>
                  <div className="input-group" style={{ marginBottom: '1.5rem' }}>
                     ...
                     <input type="email" className="input-field" value={user.email} onChange={(e) => setUser({...user, email: e.target.value})} style={{ width: '100%', padding: '0.75rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px' }} required />
                  </div>
      ...
      <div className="page-content" style={{ overflowY: 'auto', paddingBottom: '90px' }}>
         ...
         <div style={{ background: 'white', padding: '2rem 1.5rem', borderBottom: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
            ...
            <h2 style={{ margin: '0 0 0.25rem 0', fontSize: '1.5rem', fontWeight: 600, color: '#1e293b' }}>{user.name}</h2>
            <div style={{ color: '#64748b', fontSize: '0.875rem', fontWeight: 500 }}>{user.phone} • {user.email}</div>
         </div>
